...
const gasgUpcomingRewards = document.querySelector('.gsng-upcoming-rewards');
...
const loadWeb3 = async () => {
    try {
        ...
        referralID && alert(`Your referral ID: ${referralID}`)
    } catch (error) {
...
const settings = async () => {
    ...
    let _getUserGasgGainStakedBalance = await (await fetch(`https://gasgains-node-deploy.herokuapp.com/api/v1/deposit/getStakesByAddress?user=${_user}`)).json();
...
const displayTransactionsList = async (_normalTransactionists) => {
    try {
        ...
        tempItems.unshift(
            `
                <div class="trans_h_m_ds">
                    <span class="trans_h1_ds">#</span>
                    <span class="trans_h2_ds">Hash</span>
                    <span class="trans_h3_ds">From</span>
                    <span class="trans_h4_ds">To</span>
                    <span class="trans_h5_ds">Txn Fee (USD)</span>
                    <span class="trans_h6_ds">Cashback</span>
                </div>
            `
        );
...
gasgClaimCashback.addEventListener('click', async e => {
    e.preventDefault();
    try {
        await GasGainsDistribution.methods.claimRewards().send({
            from: user,
        });
        return alert("Transaction successful");
    } catch (error) {
...
gcbClaimCashback.addEventListener('click', async e => {
    e.preventDefault();
    try {
        console.log(GcbVault.methods);
        const _getMaxAmount = await GcbVault.methods.getMax(user).call();
        console.log(_getMaxAmount);
        console.log(
            await GcbVault.methods.gteclaim("1").call()
        );
        // await GcbVault.methods.claim().send({
        //     from: user
        // });
        // return alert("Transaction successful");
    } catch (error) {
